...
public interface HolidayRepository extends CrudRepository<Holiday, Long> {
    ...
    @Query("SELECT h.name FROM tb_holiday h " +
            ...
            "WHERE SUBSTRING(t.id::text, 1, 2) = :territoryIdPrefix " +
            "AND h.day = :day " +
            "AND h.month = :month " +
            "AND h.year = :year " +
            "LIMIT 1")
    Optional<String> findHolidayNameByTerritoryIdPrefixAndDate(String territoryIdPrefix, int day, int month, int year);
}